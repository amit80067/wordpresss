...
if (! class_exists ( 'DTMegaMenuPostType' ) ) {
	...
	class DTMegaMenuPostType {
		...
		function dt_register_cpt() {
			...
			$labels = array (
				'name'				 => __( 'Mega Menus', 'dt-elementor' ),
				'singular_name'		 => __( 'Mega Menu', 'dt-elementor' ),
				'menu_name'			 => __( 'Mega Menus', 'dt-elementor' ),
				'add_new'			 => __( 'Add Mega Menu', 'dt-elementor' ),
				'add_new_item'		 => __( 'Add New Mega Menu', 'dt-elementor' ),
				'edit'				 => __( 'Edit Mega Menu', 'dt-elementor' ),
				'edit_item'			 => __( 'Edit Mega Menu', 'dt-elementor' ),
				'new_item'			 => __( 'New Mega Menu', 'dt-elementor' ),
				'view'				 => __( 'View Mega Menu', 'dt-elementor' ),
				'view_item' 		 => __( 'View Mega Menu', 'dt-elementor' ),
				'search_items' 		 => __( 'Search Mega Menus', 'dt-elementor' ),
				'not_found' 		 => __( 'No Mega Menus found', 'dt-elementor' ),
				'not_found_in_trash' => __( 'No Mega Menus found in Trash', 'dt-elementor' ),
			);
		...
		function dt_template_include($template) {
			if ( is_singular( 'dt_mega_menus' ) ) {
				if ( ! file_exists ( get_stylesheet_directory () . '/single-dt_mega_menus.php' ) ) {
					$template = plugin_dir_path ( __FILE__ ) . 'templates/single-dt_mega_menus.php';
				}
			}
        ...
        function custom_fields( $item_id, $item, $depth, $args ) {
        	...
        	<p class="field-dt-mega-menu-width description description-thin">
        		<label for="edit-menu-item-dt-mega-menu-width-<?php echo esc_attr( $item_id ); ?>">
        			<?php esc_html_e( 'Mega Menu Width (px)','augury' ); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('Please set Mega Menu Width',  'dt-elementor'); ?></span>        			
        		</label>
        	</p>
        	...
        	<p class="field-dt-mega-menu-position description description-wide">
        		<label for="edit-menu-item-dt-mega-menu-position-<?php echo esc_attr( $item_id ); ?>">
        			<?php esc_html_e( 'Mega Menu Position','augury' ); ?><br />
        			<select id="edit-menu-dt-mega-menu-position-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-dt-mega-menu-position" name="menu-item-dt-mega-menu-position[<?php echo esc_attr( $item_id ); ?>]">
                        <option value="none" <?php selected( $position, '' );?>><?php esc_html_e('None','dt-elementor');?></option>
                        <option value="left" <?php selected( $position, 'left' );?>><?php esc_html_e('Left','dt-elementor');?></option>
                        <option value="right" <?php selected( $position, 'right' );?>><?php esc_html_e('Right','dt-elementor');?></option>
                        <option value="center" <?php selected( $position, 'center' );?>><?php esc_html_e('Top - Center','dt-elementor');?></option>
        			</select>
                    <span class="description"><?php esc_html_e('Please set Mega Menu Position',  'dt-elementor'); ?></span>        			
        		</label>
        	</p><?php
        }
        ...
        function dt_nav_menu_objects( $items, $args ) {
            ...
            foreach ( $items as $item ) {
                if( is_a( $args->walker, 'Augury_Walker_Nav_Menu' ) ) {
        ...
        function add_classes( $classes, $item, $args, $depth  ) {
        	...
        	if ($item->object == 'dt_mega_menus' && $depth == 1) {
                ...
                if( is_a( $args->walker, 'Augury_Walker_Nav_Menu' ) || $args->theme_location == 'main-menu' ) {
