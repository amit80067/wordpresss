<?php
class Augury_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'dt-elementor')
		);

		parent::__construct(false,AUGURY_NAME.esc_html__(' Posts','dt-elementor'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','dt-elementor');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'dt-elementor');?>
        </p>  
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						$image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'augury-blog-thumb', false);
						$image = ( $image != false)? $image[0] : AUGURY_URI . '/images/dummy-images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".augury_excerpt(10)."</div>";
					endif;
			 ...
			 endwhile;
			 	...
			 	echo "<li><h4>".esc_html__('No Posts found','dt-elementor')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";
