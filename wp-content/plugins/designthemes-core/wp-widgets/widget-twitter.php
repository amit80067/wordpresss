<?php
class Augury_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array( 
			"classname"   => 'widget_tweetbox',
			'description' => esc_html__('To show latest twitter tweets', 'dt-elementor')
		);
		parent::__construct(false,AUGURY_NAME.esc_html__(' Twitter Widget','dt-elementor'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance, array(
			'title'               => esc_html__('Latest Tweets','dt-elementor'),
			'count'               => '3',
			'username'            => '',
			'exclude_replies'     =>'1',
			'time'                =>'1',
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('consumer_key')); ?>">
        		<?php esc_html_e('Consumer Key:','dt-elementor');?>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('consumer_secret')); ?>">
        		<?php esc_html_e('Consumer Secret:','dt-elementor');?> 
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>">
        		<?php esc_html_e('Access Token:','dt-elementor');?>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('access_token_secret')); ?>">
        		<?php esc_html_e('Access Token Secret:','dt-elementor');?>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('username')); ?>">
        		<?php esc_html_e('Enter your twitter username:','dt-elementor');?>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('count')); ?>">
        		<?php esc_html_e('How many entries do you want to show:','dt-elementor');?>
        ...
        <p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('exclude_replies')); ?>">
				<?php esc_html_e('Exclude @replies','dt-elementor');?>
			</label>
		</p>
        ...
        <p>
        	...
        	<label for="<?php echo esc_attr($this->get_field_id('time')); ?>">
        		<?php esc_html_e('Show time of tweet','dt-elementor');?>
        	</label>
        </p>
        ...
        <p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('display_avatar')); ?>">
				<?php esc_html_e('Show user avatar','dt-elementor');?>
			</label>
		</p><?php
	}
	...
	function widget($args, $instance) {
		...
		echo augury_before_after_widget( $before_widget );

			if ( !empty( $title ) ) {

				echo augury_widget_title( $before_title . $title . $after_title );				
			};
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
						...
						echo '<li>';
								if( $display_avatar )
									echo '<span class="tweet-thumb">
										<a href="https://twitter.com/'.$username.'" title="'.esc_attr( 'Username', 'dt-elementor' ).'">
											<img src="'.esc_url( $tweet->user->profile_image_url ).'" alt="'.esc_attr__( 'Image', 'dt-elementor' ).'"/>
										</a></span>';
					...
					echo '<li>'.esc_html__('No public Tweets found','dt-elementor').'</li>';
				}
				echo "</ul>";
			}
		echo augury_before_after_widget( $after_widget ); 
