<?php 
class Augury_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			"classname"   => 'widget_flickr',
			'description' => esc_html__('A widget that show last flickr photo streams', 'dt-elementor')
		);

		parent::__construct( false, AUGURY_NAME.esc_html__(' Flickr Widget','dt-elementor'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr( $this->get_field_id('flickr_id') ); ?>">
        		<?php esc_html_e('Flickr ID:','dt-elementor');?>
        ...
        <p>
         ...
         <p>
         	<label for="<?php echo esc_attr($this->get_field_id('show'));?>">
         		<?php esc_html_e('What pictures to display','dt-elementor'); ?>
         		<select class="widefat" id="<?php echo esc_attr($this->get_field_id('show')); ?>" name="<?php echo esc_attr($this->get_field_name('show')); ?>"><?php
         			$a = array(
						"latest" => esc_html__("Latest",'dt-elementor'),
						"random" => esc_html__("Random",'dt-elementor')
         			);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 't_width' ); ?>">
				<?php _e( 'Thumbnail width', 'dt-elementor' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 't_height' ); ?>">
				<?php _e( 'Thumbnail height', 'dt-elementor' ); ?>:</label>
				...
				<small class="howto"><?php _e( 'Note: You can use "0" value for auto height', 'dt-elementor' ); ?></small>
		</p><?php
	}
