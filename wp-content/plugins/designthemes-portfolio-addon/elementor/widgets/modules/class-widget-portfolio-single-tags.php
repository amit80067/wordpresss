...
class Elementor_Portfolio_Single_Tags extends DTPortfolioElementorWidgetBase {
	public function get_name() {
		return 'dt-portfolio-single-tags';
	}

	public function get_title() {
		return __( 'Portfolio Single - Tags', 'dtportfolio' );
	}
			...
			$output .= '<div class="dtportfolio-terms-holder '.esc_attr($settings['class']).'">';
				...
				if( $settings['term'] == 'category' ) {
					if($settings['with_icon']) {
						$output .= get_the_term_list($settings['portfolio_id'], 'portfolio_entries', '<p class="dtportfolio-category"><i class="fas fa-tags"></i>', ', ', '</p> ');
					} else {
						$output .= get_the_term_list($settings['portfolio_id'], 'portfolio_entries', '<p class="dtportfolio-category">', ', ', '</p>');
					}
				}

				if( $settings['term'] == 'tag' ) {
					if($settings['with_icon']) {
						$output .= get_the_term_list($settings['portfolio_id'], 'portfolio_tags', '<p class="dtportfolio-tags"><i class="fas fa-tags"></i>', ', ', '</p> ');
					} else {
						$output .= get_the_term_list($settings['portfolio_id'], 'portfolio_tags', '<p class="dtportfolio-tags">', ', ', '</p>');
					}
				}				
