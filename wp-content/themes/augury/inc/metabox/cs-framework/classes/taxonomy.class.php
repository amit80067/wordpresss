 ...
 *
 */
class DTMFramework_Taxonomy extends DTMFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    $this->options = apply_filters( 'dtm_taxonomy_options', $options );
  ...
  public static function instance( $options = array() ) {
    if ( is_null( self::$instance ) && DTM_ACTIVE_TAXONOMY ) {
  ...
  public function add_taxonomy_fields() {
    ...
    foreach ( $this->options as $option ) {
      ...
      $get_taxonomy = dtm_get_var( 'taxonomy' );
  ...
  public function render_taxonomy_form_fields( $term ) {
    ...
    $cs_errors = get_transient( 'dtm-taxonomy-transient' );

    wp_nonce_field( 'dtm-taxonomy', 'dtm-taxonomy-nonce' );

    echo '<div class="cs-framework dtm-taxonomy dtm-taxonomy-'. $classname .'-fields">';
      ...
      foreach( $this->options as $option ) {
        ...
        if( $taxonomy == $option['taxonomy'] ) {
          ...
          foreach ( $option['fields'] as $field ) {
            ...
            echo dtm_add_element( $field, $elem_value, $option['id'] );
  ...
  public function save_taxonomy( $term_id ) {
    ...
    if ( wp_verify_nonce( dtm_get_var( 'dtm-taxonomy-nonce' ), 'dtm-taxonomy' ) ) {

      $errors = array();
      $taxonomy = dtm_get_var( 'taxonomy' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( $taxonomy == $request_value['taxonomy'] ) {
            ...
            foreach( $request_value['fields'] as $field ) {
              ...
              if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                ...
                if( has_filter( 'dtm_sanitize_'. $sanitize_type ) ) {
                  $request[$field['id']] = apply_filters( 'dtm_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
                }

                // validate options
                if ( isset( $field['validate'] ) && has_filter( 'dtm_validate_'. $field['validate'] ) ) {

                  $validate = apply_filters( 'dtm_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
          ...
          $request = apply_filters( 'dtm_save_taxonomy', $request, $request_key, $term_id );
      ...
      set_transient( 'dtm-taxonomy-transient', $errors, 10 );
