 ...
 *
 */
class DTMFramework_Metabox extends DTMFramework_Abstract{
  ...
  public function __construct( $options ){
    ...
    $this->options = apply_filters( 'dtm_metabox_options', $options );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && DTM_ACTIVE_METABOX ) {
  ...
  public function render_meta_box_content( $post, $callback ) {
    ...
    wp_nonce_field( 'dtm-framework-metabox', 'dtm-framework-metabox-nonce' );

    $unique     = $callback['args']['id'];
    $sections   = $callback['args']['sections'];
    $meta_value = get_post_meta( $post->ID, $unique, true );
    $transient  = get_transient( 'dtm-metabox-transient' );
    ...
    $section_id = dtm_get_var( 'cs-section', $section_id );
          ...
          foreach( $sections as $v ) {
            ...
            if( isset( $v['fields'] ) ) {
              ...
              foreach ( $v['fields'] as $field_key => $field ) {
                ...
                echo dtm_add_element( $field, $elem_value, $unique );
  ...
  public function save_post( $post_id, $post ) {
    ...
    if ( wp_verify_nonce( dtm_get_var( 'dtm-framework-metabox-nonce' ), 'dtm-framework-metabox' ) ) {

      $errors = array();
      $post_type = dtm_get_var( 'post_type' );
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( in_array( $post_type, (array) $request_value['post_type'] ) ) {
          ...
          $request = dtm_get_var( $request_key, array() );
            ...
            if( isset( $section['fields'] ) ) {
              ...
              foreach( $section['fields'] as $field ) {
                ...
                if( isset( $field['type'] ) && isset( $field['id'] ) ) {
                  ...
                  $field_value = dtm_get_vars( $request_key, $field['id'] );
                  ...
                  if( has_filter( 'dtm_sanitize_'. $sanitize_type ) ) {
                    $request[$field['id']] = apply_filters( 'dtm_sanitize_' . $sanitize_type, $field_value, $field, $section['fields'] );
                  }

                  // validate options
                  if ( isset( $field['validate'] ) && has_filter( 'dtm_validate_'. $field['validate'] ) ) {

                    $validate = apply_filters( 'dtm_validate_' . $field['validate'], $field_value, $field, $section['fields'] );
          ...
          $request = apply_filters( 'dtm_save_post', $request, $request_key, $post );
          ...
          $transient['ids'][$request_key] = dtm_get_vars( 'cs_section_id', $request_key );
      ...
      set_transient( 'dtm-metabox-transient', $transient, 10 );
