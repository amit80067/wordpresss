 ...
 *
 */
abstract class DTMFramework_Options extends DTMFramework_Abstract {
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'DTM_OPTIONS_DEBUG' ) && DTM_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". esc_html__( 'CONFIG', 'augury' ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". esc_html__( 'USAGE', 'augury' ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "dtm_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". esc_html__( 'VALUE', 'augury' ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'DTM_OPTIONS_DEBUG_LIGHT' ) && DTM_OPTIONS_DEBUG_LIGHT ) ) {
      ...
      $out .= ( isset( $this->field['id'] ) ) ? "dtm_get_option( '". $this->field['id'] ."' );" : '';
      $out .= "\n";
      $out .= "<strong>". esc_html__( 'ID', 'augury' ) .":</strong>";
      $out .= "\n";
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      foreach ( $this->multilang['languages'] as $key => $val ) {
        ...
        if( $key != $this->multilang['current'] ) {
          ...
          $class = 'DTMFramework_Option_' . $this->field['type'];
      ...
      $out .= '</fieldset>';
      $out .= '<p class="cs-text-desc">'. sprintf( esc_html__( 'You are editing language: ( %s )', 'augury' ), '<strong>'. $this->multilang['current'] .'</strong>' ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? dtm_language_defaults() : false;
  }

}

// load all of fields
dtm_load_option_fields();
