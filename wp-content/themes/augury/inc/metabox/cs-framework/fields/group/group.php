 ...
 *
 */
class DTMFramework_Option_group extends DTMFramework_Options {
  ...
  public function output() {
    ...
    $acc_title   = ( isset( $this->field['accordion_title'] ) ) ? $this->field['accordion_title'] : esc_html__( 'Adding', 'augury' );
    ...
    $search_id   = dtm_array_search( $fields, 'id', $acc_title );
    ...
    echo '<div class="cs-group cs-group-'. $el_class .'-adding hidden">';
      ...
      foreach ( $fields as $field ) {
        ...
        echo dtm_add_element( $field, $field_default, $unique );
      }
      echo '<div class="cs-element cs-text-right cs-remove"><a href="#" class="button cs-warning-primary cs-remove-group">'. esc_html__( 'Remove', 'augury' ) .'</a></div>';
      echo '</div>';
      ...
      if( ! empty( $this->value ) ) {
        ...
        foreach ( $this->value as $key => $value ) {
          ...
          foreach ( $fields as $field ) {
            ...
            echo dtm_add_element( $field, $value, $unique );
          }
