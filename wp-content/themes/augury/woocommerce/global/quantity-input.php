...
if ( $max_value && $min_value === $max_value ) {
	...
	$label = ! empty( $args['product_name'] ) ? sprintf( esc_html__( '%s quantity', 'augury' ), wp_strip_all_tags( $args['product_name'] ) ) : esc_html__( 'Quantity', 'augury' );

	/* Customized script */
	$woo_quantity_plusnminus = augury_get_option( 'dt-woo-quantity-plusnminus' );
	...
	<div class="quantity <?php echo esc_attr($plusminus_class); ?>">
		...
		<input
			...
			title="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'augury' ); ?>"
			size="4"
