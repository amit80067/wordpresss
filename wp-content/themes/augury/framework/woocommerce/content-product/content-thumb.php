...
if( ! function_exists( 'augury_woo_loop_product_thumbnail' ) ) {
	
	function augury_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_styles != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.augury_html_output($product_overlay_bgcolor).'></div>';
				}
			...
			if($product_thumb_content) {
				...
				augury_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.augury_html_output($product_thumb_content_overlay_bgcolor).'>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'augury_woo_loop_product_thumbnail', 10 );
